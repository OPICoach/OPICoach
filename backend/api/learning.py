...
class NoteGenerationRequest(BaseModel):
    user_pk: int = Field(..., description="사용자 PK (DB 기본키)")
    topic: str = Field(..., description="노트 생성할 주제")
    LLM_model: str = Field("gemini-2.0-flash", description="gemini-2.5-pro-preview-05-06 or gemini-2.0-flash")
...
@router.post("/notes/generate", response_model=NoteGenerationResponse)
async def api_generate_note(request: NoteGenerationRequest):
    """주제에 대한 학습 노트를 생성합니다."""
    try:
        ...
        print(f"Error in /learning/notes/generate: {e}")
        raise HTTPException(status_code=500, detail="노트 생성 중 오류가 발생했습니다.")
