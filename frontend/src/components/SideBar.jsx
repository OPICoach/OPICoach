...
const SideBar = ({ userName }) => {
  ...
  return (
    <aside className="flex flex-col w-60 h-screen bg-neutral">
      {/* 로고 */}
      ...
      <nav className="flex-1 mt-8 mx-1">
        {menus.map((menu) => (
          <a
            key={menu.name}
            href={menu.path}
            className="flex items-center my-[10px] mx-[5px] px-4 py-3 text-accent border-[#E5E7EB] rounded-lg hover:bg-white transition"
          >
            <img
              src={menu.icon}
              alt={menu.name}
